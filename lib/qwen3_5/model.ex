defmodule Qwen3_5.Model do
  ...
  #
  # `:adapt` overrides the matcher outright. It is fetched lazily, so passing
  # one means `:rank`, `:alpha`, and the application environment are never
  # consulted — the matcher already carries whatever it wants to adapt.
  ...
  defp layer_plans(config, head_width, kv_width) do
    layer = %{
      ...
      attention: %{
        q_proj: {:tensor, :normal, {head_width, config.hidden}},
        k_proj: {:tensor, :normal, {kv_width, config.hidden}},
        v_proj: {:tensor, :normal, {kv_width, config.hidden}},
        o_proj: {:tensor, :normal, {config.hidden, head_width}},
  ...
  defp fill(:normal, shape, key, config) do
